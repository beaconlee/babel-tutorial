...
Vector3d left_straight_left(double x, double y, double phi, bool& flag)
{
  ...
  if(ut[1] >= 0.0)
  {
    double v = beacon::Mod2Pi(phi - ut[1]);
    if(v >= 0.0)
    {
      flag = true;
      return {ut[1], ut[0], v};
...
Vector3d left_straight_right(double x, double y, double phi, bool& flag)
{
  ...
  if(u1 >= 4.0)
  {
    ...
    double t = beacon::Mod2Pi(ut1[1] + theta);
    double v = beacon::Mod2Pi(t - phi);
...
Vector3d left_right_left(double x, double y, double phi, bool& flag)
{
  ...
  if(ut1[0] <= 4.0)
  {
    ...
    double t = beacon::Mod2Pi(ut1[1] + 0.5 * u + M_PI);
    double v = beacon::Mod2Pi(phi - t + u);
...
void set_path(vector<Path>& paths,
              Vector3d lengths,
              vector<char> ctypes,
              double step_size)
{
...
void straight_curve_straight(
    double x, double y, double phi, vector<Path>& paths, double step_size)
{
  bool flag = false;
...
vector<Path>
generate_path(Vector3d q0, Vector3d q1, double max_curvature, double step_size)
{
  double dx = q1[0] - q0[0];
  double dy = q1[1] - q0[1];
  ...
  std::cout << __FUNCTION__ << "  " << __LINE__
            << " pahts.size(): " << paths.size() << "\n";

  std::cout << __FUNCTION__ << "  " << __LINE__ << "\n";
  for(auto path : paths)
  {
    std::cout << " path.length: " << path.lengths.size() << "\n";
    std::cout << "len: ";
...
vector<vector<double>> calc_interpolate_dists_list(vector<double> lengths,
                                                   double step_size)
{
  ...
  for(double length : lengths)
  {
    ...
    int len_Sign = beacon::Sign(length);
...
Vector4d interpolate(double dist,
                     double length,
                     char mode,
...
vector<vector<double>> generate_local_course(vector<double> lengths,
                                             vector<char> modes,
                                             double max_curvature,
                                             double step_size)
{
...
vector<Path>
calc_rs_paths(Vector3d s, Vector3d g, double maxc, double step_size)
{
  ...
  for(Path& path : paths)
  {
    vector<vector<double>> states = generate_local_course(
        path.lengths, path.ctypes, maxc, step_size * maxc);
    for(size_t idx = 0; idx < states[0].size(); ++idx)
    {
      ...
      path.yaw.emplace_back(beacon::Mod2Pi(yaw + s[2]));
      path.directions.emplace_back(direction);
    }

    std::cout << "解析转换后的坐标\n";
    for(auto x : path.directions)
    {
      std::cout << x << " ";
    }
    std::cout << "\n";
